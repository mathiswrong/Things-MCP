...
Individual to-do Trash is destructive and requires its separate grant. Status changes, notes replacement, and project changes can also affect data. Container and permanent deletion, restoration, and a general undo mechanism are not implemented. Revision checks cover exposed fields, not hidden checklists or child membership. Project completion and movement may affect descendants; populated-project effects have not been fully verified. Review `things_capabilities` before granting an application access.
