...
The initial local stdio foundation now includes eight tools, a supported AppleScript adapter, strict schemas, default read-only permission, revision checks, read-back verification, a shared filesystem lease, durable request receipts, and scrubbed optional Sentry reporting. Synthetic tests cover ordinary mutation behavior and failure handling. Real read-only health, search, and ID lookup were verified against Things 3.23.3 through the bundled executable and official MCP client. The actual assistant applications have not yet been verified. Native mutations remain unverified and disabled by default. No personal Things data was changed.

For the first CLI release, one stdio process runs per client. A maintained filesystem-lock library and shared state directory coordinate all mutations. Ownership checks and cancellation protect against continuing after a lost lease. This replaces the initially proposed background daemon for the feasibility milestone and avoids adding a service lifecycle before it is needed. All clients must use the same state directory. A daemon, per-client grants, Shortcuts bridge, remote connectivity, and native settings UI remain later work.

The feasibility milestone is not complete until isolated-library mutations, heading/checklist round trips, and actual use in both initial assistant clients are demonstrated. Refer to README.md and the runtime capability registry for current behavior.
...
6. **Make setup and maintenance part of the product.**
   ...
   Deliver a CLI first, a Claude `.mcpb` package, and eventually a signed/notarized Mac installer with a small status/settings menu. Bundle runtime dependencies for nontechnical users. Add a native helper only if needed for stable permission identity, Keychain access, or URL callbacks; prove that need before adding another language and build system.
