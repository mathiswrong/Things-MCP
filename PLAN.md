...
Setup revision: the owner rejected manual configuration and Terminal instructions as the user installation experience. The next milestone is the native setup proposal in [SETUP-PLAN.md](SETUP-PLAN.md), requiring both ChatGPT desktop and ordinary browser access from the start. Artboards are ready for review; the native installer and connection manager have not been implemented. Existing manual commands below remain development instructions, not acceptance of the user setup experience.
